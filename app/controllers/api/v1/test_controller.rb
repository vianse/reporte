...
@año = Date.today.year
 ...
 @crear_ordenes = Facturada.create(facturas_params)
  render :json => @crear_ordenes.to_json(:only => [:orden, :tipo, :importe, :fecha, :estatus,:app_id]) 
 ...
 private
  def facturas_params
   ...
   ActionController::Parameters.new(p.to_hash).permit(:orden, :tipo, :importe, :fecha, :estatus,:app_id) #, :factura,:mes,:año
   end
  end
end